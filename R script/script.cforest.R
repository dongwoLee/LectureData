...
cForestMod <- cforest(Mosq ~ ., data = trainData)
  rffile <- paste(args[1],".cforest.pmosq.log.Rdata",sep="")
  save(cForestMod,file=rffile)

# 평가: 학습데이터로 평가
actuals <- trainData$Mosq
predicted <- predict(cForestMod, newdata = trainData, type="prob")
...
predicted <- predict(cForestMod, newdata = testData, type="prob")
  probs <- t(data.frame(matrix(unlist(predicted),nrow=8)))
  #probs <- t(as.data.frame(predicted))
  rownames(probs) <- NULL
  predicted <- t(apply(probs,1,function(row) { 
...
cForestMod <- cforest(Mosq ~ ., data = trainData)
  rffile <- paste(args[1],".cforest.pmosq.Rdata",sep="")
  save(cForestMod,file=rffile)
...
# 데이터 학습(전날모기데이터 + 로그미리 적용)
cForestMod <- cforest(Mosq ~ ., data = trainData)
  rffile <- paste(args[1],".cforest.log.Rdata",sep="")
  save(cForestMod,file=rffile)
...
# 데이터 학습(전날모기데이터 + 로그미리 적용안함)
cForestMod <- cforest(Mosq ~ ., data = trainData)
  rffile <- paste(args[1],".cforest.Rdata",sep="")
  save(cForestMod,file=rffile)
